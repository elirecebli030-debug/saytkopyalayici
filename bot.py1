...
CHANNEL_INVITE_LINK = os.environ.get("CHANNEL_INVITE_LINK", "https://t.me/+oI3oBH9ESxsyNTUy")
...
async def db_init(app):
    ...
    async with POOL.acquire() as conn:
        ...
        # köhnə verisyadan qalma quraşdırmalar üçün təhlükəsiz miqrasiya
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS verified BOOLEAN NOT NULL DEFAULT FALSE")
...
async def db_is_verified(user_id: int) -> bool:
    async with POOL.acquire() as conn:
        val = await conn.fetchval("SELECT verified FROM users WHERE user_id=$1", user_id)
        return bool(val)


async def db_mark_verified(user_id: int):
    async with POOL.acquire() as conn:
        await conn.execute(
...
async def db_register_referral(referred_user_id: int, referrer_id: int) -> bool:
    """Referral-ı sayır (qorumasız — hər /start referral kimi sayılır)."""
    async with POOL.acquire() as conn:
        await conn.execute(
            """
...
def mask_username(user) -> str:
    """Açıq kanal bildirişləri üçün istifadəçi adının yalnız ilk 3 hərfini göstərir."""
    base = user.username or user.full_name or "İstifadəçi"
    return f"@{base[:3]}***" if user.username else f"{base[:3]}***"
...
async def ensure_verified(context: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    """Kanal-üzvlüyü YALNIZ BİR DƏFƏ Telegram-dan yoxlayır — təsdiqlənəndən sonra
    DB-də saxlanılır və artıq hər mesajda təkrar yoxlanmır (sürətli, az API çağırışı)."""
    if await db_is_verified(user_id):
        return True
    if await is_subscribed(context, user_id):
        await db_mark_verified(user_id)
        return True
    return False
...
async def check_domain_resolves(domain: str) -> bool:
    """Domenin DNS-də mövcud olub-olmadığını tez yoxlayır (host tapılmırsa sayt yoxdur)."""
    try:
        await asyncio.to_thread(socket.gethostbyname, domain.split(":")[0])
        return True
    except Exception:
        return False


def diagnose_clone_error(error_text: str) -> str:
    """wget-in xəta çıxışını insan-dilinə çevirir ki, istifadəçi problemi anlasın."""
    low = (error_text or "").lower()
    if any(k in low for k in ("403", "forbidden", "cloudflare", "captcha", "access denied", "blocked", "999", "attention required")):
        return "Bu sayt bot/scraper əleyhinə qoruma (WAF, Cloudflare və ya bənzəri) tətbiq edir — ona görə klonlana bilmir."
    if any(k in low for k in ("429", "too many requests", "rate limit")):
        return "Sayt sürət limiti qoyub (çox sorğu göndərildi deyə bloklanıb) — bir az sonra yenidən cəhd edin."
    if any(k in low for k in ("timed out", "connection refused", "unable to connect", "no route to host", "network is unreachable")):
        return "Sayt serverinə qoşulmaq mümkün olmadı — server cavab vermir və ya əlçatan deyil."
    if any(k in low for k in ("name or service not known", "temporary failure in name resolution", "could not resolve")):
        return "Bu domen tapılmadı — belə bir sayt mövcud deyil."
    if "certificate" in low or "ssl" in low:
        return "Saytın SSL sertifikatı ilə bağlı problem var, ona görə qoşulmaq mümkün olmadı."
    return error_text or "naməlum xəta"
...
async def maybe_register_referral(context: ContextTypes.DEFAULT_TYPE, user):
    ...
    if referrer_id is None:
        return
    await db_register_referral(user.id, referrer_id)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id not in ADMIN_IDS and not await ensure_verified(context, user.id):
...
async def check_sub_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await ensure_verified(context, user.id):
...
async def handle_domain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS and not await ensure_verified(context, user_id):
        await show_gate(update, context)
        return
    ...
    if not await check_domain_resolves(domain):
        await update.message.reply_text(
            f"❌ *{domain}* tapılmadı — belə bir sayt mövcud deyil (domen DNS-də qeydə alınmayıb).",
            parse_mode="Markdown",
        )
        return
    ...
    if not success:
        if not is_repeat:
            shutil.rmtree(clone_dir, ignore_errors=True)
        friendly_reason = diagnose_clone_error(error_text)
        await status_msg.edit_text(
            f"❌ *{domain}* kopyalanmadı.\nSəbəb: {friendly_reason}",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([]),
        )
        await log_event(context, f"❌ {user_label(user)} — `{domain}` klonlanmadı: {friendly_reason}")
        return
...
async def successful_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if CHANNEL_ID:
        try:
            await context.bot.send_message(
                chat_id=CHANNEL_ID,
                text=f"💰 *{paid_domain}* saytı {mask_username(buyer)} tərəfindən *{payment.total_amount} ⭐*-a klonlanıb alınıb.",
                parse_mode="Markdown",
            )
        except Exception as e:
            logger.warning(f"Kanala ödəniş bildirişi göndərilmədi: {e}")
    await send_clone_zip(context, update.effective_chat.id, clone_dir, slug)
